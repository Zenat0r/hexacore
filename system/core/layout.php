...
class Layout
{
	...
	public function __construct($argument){
		...
		$this->fonts = array();
		$this->scripts = array();
		$this->og_tag = array("title" => null, "description" => null, "img" => null, "url" => null);
	...
	private function og_url(){
		...
		if($action === null && $gets !== null){
			...
			if($gets !== null) $this->og_tag["url"] .= "/".$gets;
		}
	}
	public function render($nameView, $data=NULL) {
	    ...
	    if (file_exists($this->name)) {
	      ...
	      $layout_data = array(
	      	'title' => $this->title != null ? $this->title : array(),
	      	'styles' => $this->styles != null ? $this->styles : array(),
	      	'fonts' => $this->fonts != null ? $this->fonts : array(),
	      	'scripts' => $this->scripts != null ? $this->scripts : array(),
	      	'remote_fonts' => $this->remote_fonts != null ? $this->remote_fonts : array(),
	      	'og_tag' => $this->og_tag,
	      	'layoutData' => $this->layoutData ,
	      	"flashData" => $this->flashData
	      );
	      if($layout_data!==NULL)extract($layout_data);
